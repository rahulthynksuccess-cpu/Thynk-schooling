...
import { Check, ArrowRight, Star, Zap, Tag, X, Loader2, AlertCircle, CheckCircle2, MapPin } from 'lucide-react'
...
const DEFAULT_FAQ = [
  ...
  { q: 'What is a Featured Listing?', a: 'Featured schools appear at the top of search results in their city with a special badge. You get significantly more parent views and enquiries.' },
]
...
interface CouponResult { valid: boolean; coupon_id?: string; code?: string; type?: string; value?: number; discount_paise?: number; final_amount_paise?: number; message: string }

const ease = [0.22, 1, 0.36, 1] as const
const fmt = (p: number) => p === 0 ? '₹0' : `₹${Math.round(p / 100).toLocaleString('en-IN')}`
    ...
    <div style={{ position:'fixed',inset:0,zIndex:100,display:'flex',alignItems:'center',justifyContent:'center',padding:16 }}>
      ...
      <motion.div initial={{ opacity:0,scale:0.96,y:20 }} animate={{ opacity:1,scale:1,y:0 }} exit={{ opacity:0,scale:0.96,y:20 }} transition={{ duration:0.28,ease }}
        ...
        <div style={{ display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:22 }}>
          ...
          <button onClick={onClose} style={{ background:'none',border:'none',cursor:'pointer',color:'rgba(250,247,242,0.4)',display:'flex',padding:4 }}><X style={{ width:18,height:18 }} /></button>
        </div>
        <div style={{ background:'rgba(255,255,255,0.04)',border:'1px solid rgba(255,255,255,0.08)',borderRadius:12,padding:'14px 16px',marginBottom:20 }}>
          <div style={{ display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:couponResult?.valid?8:0 }}>
            ...
            <span style={{ fontFamily:'"Cormorant Garamond",serif',fontWeight:700,fontSize:18,color:'#FAF7F2' }}>{fmt(plan.price)}</span>
          </div>
          {couponResult?.valid && <>
            <div style={{ display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:8 }}>
              <span style={{ fontFamily:'Inter,sans-serif',fontSize:13,color:'#4ADE80' }}>Discount ({couponResult.code})</span>
              <span style={{ fontFamily:'Inter,sans-serif',fontSize:13,fontWeight:600,color:'#4ADE80' }}>−{fmt(couponResult.discount_paise!)}</span>
            </div>
            ...
            <div style={{ display:'flex',justifyContent:'space-between',alignItems:'center' }}>
              ...
              <span style={{ fontFamily:'"Cormorant Garamond",serif',fontWeight:700,fontSize:22,color:'#E8C547' }}>{fmt(effectiveAmount)}</span>
            </div>
          </>}
        </div>
            ...
            <>
              <div style={{ display:'flex',gap:8 }}>
                ...
                <button onClick={validateCoupon} disabled={!couponCode.trim()||couponLoading}
                  style={{ padding:'11px 16px',borderRadius:10,border:'1px solid rgba(184,134,11,0.4)',background:'rgba(184,134,11,0.1)',color:'#E8C547',fontSize:13,fontWeight:600,cursor:couponCode.trim()?'pointer':'not-allowed',fontFamily:'Inter,sans-serif',display:'flex',alignItems:'center',gap:6,opacity:couponCode.trim()?1:0.5,transition:'opacity .15s' }}>
        ...
        <Link href={checkoutUrl} style={{ display:'flex',alignItems:'center',justifyContent:'center',gap:8,padding:'14px 24px',borderRadius:12,background:'linear-gradient(135deg,#B8860B,#E8C547)',color:'#0D1117',fontFamily:'Inter,sans-serif',fontSize:14,fontWeight:700,textDecoration:'none',transition:'opacity .15s' }}>
          Proceed to Pay {fmt(effectiveAmount)} <ArrowRight style={{ width:14,height:14 }} />
        </Link>
...
export default function PricingPage() {
  ...
  const { data: subPlans,      isLoading: plansLoading }    = useQuery<SubPlan[]>({ queryKey: ['subscription-plans'], queryFn: () => fetch('/api/admin?action=subscription-plans').then(r => r.json()), staleTime: 5 * 60 * 1000 })
  const { data: featuredData,  isLoading: featuredLoading } = useQuery<{ plans: FeaturedPlan[] }>({ queryKey: ['featured-listing-plans'], queryFn: () => fetch('/api/featured-listing-plans').then(r => r.json()), staleTime: 5 * 60 * 1000 })
  ...
  const activeFeaturedPlans = (featuredData?.plans ?? []).filter(p => p.isActive).sort((a, b) => a.sortOrder - b.sortOrder)

  const FAQ = DEFAULT_FAQ.map((def, i) => ({ q: pricingContent?.[`faq${i+1}q`] || def.q, a: pricingContent?.[`faq${i+1}a`] || def.a }))
  const desktopCols = Math.min(Math.max(activeLeadPlans.length, 1), 4)
  ...
  return (
    <>
      <style>{`
        ...
        .pc-card { background:#F5F0E8;border:1px solid rgba(13,17,23,0.08);border-radius:18px;padding:28px 24px 24px;display:flex;flex-direction:column;position:relative;overflow:hidden;transition:transform 0.22s cubic-bezier(.22,1,.36,1),box-shadow 0.22s cubic-bezier(.22,1,.36,1);cursor:default; }
      ...
      <main className="pricing-page-root" style={{ paddingTop:72 }}>
        ...
        <section style={{ position:'relative',padding:'clamp(72px,10vw,120px) 0 clamp(56px,8vw,96px)',display:'flex',alignItems:'center',overflow:'hidden' }}>
          ...
          <div style={{ maxWidth:1200,margin:'0 auto',padding:'0 clamp(20px,5vw,72px)',width:'100%',position:'relative',zIndex:1 }}>
            ...
            <motion.h1 initial={{ opacity:0,y:28 }} animate={{ opacity:1,y:0 }} transition={{ duration:0.7,delay:0.08,ease }}
              ...
              Simple Pricing,<em className="shimmer-text" style={{ display:'block',fontStyle:'italic' }}>Powerful Results</em>
            </motion.h1>
            <motion.p initial={{ opacity:0,y:14 }} animate={{ opacity:1,y:0 }} transition={{ duration:0.6,delay:0.18,ease }}
              ...
              List free. Get lead credits with a plan. Boost visibility with featured listing. No lock-in.
            </motion.p>
            <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }} transition={{ delay:0.32,duration:0.55 }}
              ...
              <div style={{ display:'flex',gap:2 }}>{[1,2,3,4,5].map(s => <Star key={s} style={{ width:13,height:13,fill:'#E8C547',color:'#E8C547' }} />)}</div>
              <span style={{ fontFamily:'Inter,sans-serif',fontSize:12,color:'rgba(250,247,242,0.4)',fontWeight:300 }}>Trusted by <strong style={{ color:'rgba(250,247,242,0.7)',fontWeight:600 }}>8,000+ schools</strong> across India</span>
            </motion.div>
                ...
                {activeTab === 'leads' ? (
                  <>
                    {plansLoading ? (
                      ...
                      <div className="plans-grid" style={{ display:'grid',gridTemplateColumns:`repeat(${desktopCols},1fr)`,gap:'clamp(10px,1.5vw,16px)',alignItems:'start' }}>
                          ...
                          <motion.div key={plan.id} initial={{ opacity:0,y:32,scale:0.96 }} animate={plansInView?{opacity:1,y:0,scale:1}:{}} transition={{ delay:i*0.09,duration:0.6,ease }}>
                            <div className={`pc-card${plan.isHot?' hot':''}`}>
                              ...
                              <div style={{ marginBottom:10,display:'flex',alignItems:'baseline',gap:4 }}>
                                <span style={{ fontFamily:'"Cormorant Garamond",serif',fontWeight:700,fontSize:'clamp(2rem,3.5vw,2.8rem)',color:plan.isHot?'#E8C547':'#0D1117',letterSpacing:'-1.5px',lineHeight:1 }}>{fmt(plan.price)}</span>
                              </div>
                              {plan.leadCount !== 0 && (
                                <div style={{ display:'inline-flex',alignItems:'center',gap:5,padding:'4px 10px',borderRadius:8,background:plan.isHot?'rgba(184,134,11,0.2)':'rgba(184,134,11,0.08)',border:`1px solid ${plan.isHot?'rgba(184,134,11,0.4)':'rgba(184,134,11,0.2)'}`,marginBottom:16,alignSelf:'flex-start' }}>
                                  <Zap style={{ width:10,height:10,color:'#B8860B' }} />
                              ...
                              <div style={{ display:'flex',flexDirection:'column',gap:9,flex:1,marginBottom:22 }}>
                                {plan.features.map(f => (
                                  <div key={f} style={{ display:'flex',alignItems:'flex-start',gap:9,fontFamily:'Inter,sans-serif',fontSize:12,color:plan.isHot?'rgba(250,247,242,0.7)':'#4A5568',fontWeight:300,lineHeight:1.45 }}>
                                    <div className={`pc-check ${plan.isHot?'gold':'light'}`}><Check style={{ width:9,height:9,color:plan.isHot?'#E8C547':'#6B7280' }} /></div>{f}
                                  </div>
                                ))}
                              </div>
                              {isSchoolUser ? (
                                <button onClick={() => setCheckoutPlan(plan)} className={plan.isHot?'btn btn-gold':'btn btn-dark'} style={{ textAlign:'center',justifyContent:'center',display:'flex',fontSize:13,width:'100%',cursor:'pointer' }}>
                                  {plan.cta} <ArrowRight style={{ width:13,height:13 }} />
                                </button>
                              ) : (
                                <Link href={`/register?role=school&plan=${plan.planKey}`} className={plan.isHot?'btn btn-gold':'btn btn-dark'} style={{ textAlign:'center',justifyContent:'center',display:'flex',fontSize:13 }}>
                                  {plan.cta} <ArrowRight style={{ width:13,height:13 }} />
                                </Link>
                              )}
                            </div>
                  ...
                  <>
                    ...
                    <div style={{ display:'flex',flexWrap:'wrap',gap:10,justifyContent:'center',marginBottom:32 }}>
                      {[{icon:'🔝',label:'Top of search'},{icon:'✅',label:'Featured badge'},{icon:'📍',label:'City priority'},{icon:'📧',label:'Parent emails'},{icon:'📊',label:'Analytics boost'}].map(b => (
                        <div key={b.label} style={{ display:'flex',alignItems:'center',gap:8,padding:'8px 16px',borderRadius:99,background:'rgba(13,17,23,0.05)',border:'1px solid rgba(13,17,23,0.1)',fontSize:13,color:'#4A3728',fontWeight:500 }}>
                    ...
                    {featuredLoading ? (
                      <div style={{ display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:16 }}>
                        {Array.from({length:3}).map((_,i) => <div key={i} style={{ height:380,borderRadius:18,background:'rgba(13,17,23,0.06)',animation:'pulse 1.5s infinite' }} />)}
                      </div>
                    ) : activeFeaturedPlans.length === 0 ? (
                      <div style={{ textAlign:'center',padding:'48px 0',color:'#9B8860' }}>
                        <div style={{ fontSize:36,marginBottom:12 }}>🌟</div>
                        <div style={{ fontWeight:600,fontSize:15,color:'#0D1117',marginBottom:6 }}>Featured plans coming soon</div>
                      </div>
                    ) : (
                      <div className="plans-grid" style={{ display:'grid',gridTemplateColumns:`repeat(${Math.min(activeFeaturedPlans.length,3)},1fr)`,gap:'clamp(10px,1.5vw,16px)',alignItems:'start',maxWidth:900,margin:'0 auto' }}>
                        {activeFeaturedPlans.map((plan, i) => (
                          <motion.div key={plan.id} initial={{ opacity:0,y:32,scale:0.96 }} animate={plansInView?{opacity:1,y:0,scale:1}:{}} transition={{ delay:i*0.09,duration:0.6,ease }}>
                            <div className={`pc-card${plan.isHot?' hot':''}`}>
                              {plan.isHot && <div className="pc-hot-badge"><Star style={{ width:9,height:9 }} /> Best Value</div>}
                              <div style={{ display:'inline-flex',alignItems:'center',gap:5,padding:'4px 10px',borderRadius:8,background:plan.isHot?'rgba(184,134,11,0.2)':'rgba(184,134,11,0.08)',border:`1px solid ${plan.isHot?'rgba(184,134,11,0.4)':'rgba(184,134,11,0.2)'}`,marginBottom:14 }}>
                                <MapPin style={{ width:10,height:10,color:'#B8860B' }} />
                                <span style={{ fontSize:11,fontWeight:700,color:'#B8860B' }}>{durationLabel(plan.durationDays)}</span>
                              </div>
                              <div style={{ marginBottom:16 }}>
                                <div style={{ fontFamily:'"Cormorant Garamond",serif',fontWeight:700,fontSize:20,color:plan.isHot?'#FAF7F2':'#0D1117',marginBottom:3 }}>{plan.name}</div>
                                <div style={{ fontFamily:'Inter,sans-serif',fontSize:11,color:plan.isHot?'rgba(250,247,242,0.45)':'#8A9AB0',fontWeight:300,lineHeight:1.5 }}>{plan.description}</div>
                              ...
                              <div style={{ marginBottom:20,display:'flex',alignItems:'baseline',gap:4 }}>
                                <span style={{ fontFamily:'"Cormorant Garamond",serif',fontWeight:700,fontSize:'clamp(2rem,3.5vw,2.8rem)',color:plan.isHot?'#E8C547':'#0D1117',letterSpacing:'-1.5px',lineHeight:1 }}>{fmt(plan.price)}</span>
                                <span style={{ fontFamily:'Inter,sans-serif',fontSize:12,color:plan.isHot?'rgba(250,247,242,0.4)':'#A0ADB8',fontWeight:300 }}>/{durationLabel(plan.durationDays)}</span>
                              </div>
                              <div style={{ height:1,background:plan.isHot?'rgba(255,255,255,0.07)':'rgba(13,17,23,0.07)',marginBottom:16 }} />
                              <div style={{ display:'flex',flexDirection:'column',gap:9,flex:1,marginBottom:22 }}>
                                {plan.features.map(f => (
                              ...
                              <Link href={isSchoolUser ? `/dashboard/school/packages?tab=featured` : `/register?role=school`} className={plan.isHot?'btn btn-gold':'btn btn-dark'} style={{ textAlign:'center',justifyContent:'center',display:'flex',fontSize:13 }}>
                                {plan.cta} <ArrowRight style={{ width:13,height:13 }} />
                              </Link>
                            </div>
                          </motion.div>
                    ...
                    <motion.p initial={{ opacity:0 }} whileInView={{ opacity:1 }} viewport={{ once:true }} transition={{ delay:0.3,duration:0.5 }}
                      style={{ textAlign:'center',marginTop:28,fontFamily:'Inter,sans-serif',fontSize:12,color:'#A0ADB8',fontWeight:300 }}>
                      Featured status activates immediately · No lock-in · Renew anytime
                    </motion.p>
                  </>
                )}
              </motion.div>
        ...
        <section ref={faqRef} style={{ background:'#0A0E17',padding:'clamp(60px,8vw,96px) 0',position:'relative' }}>
          ...
          <div style={{ maxWidth:740,margin:'0 auto',padding:'0 clamp(20px,5vw,40px)',position:'relative',zIndex:1 }}>
            <motion.div initial={{ opacity:0,y:16 }} animate={faqInView?{opacity:1,y:0}:{}} transition={{ duration:0.55,ease }} style={{ textAlign:'center',marginBottom:36 }}>
        ...
        <section style={{ background:'#F5F0E8',padding:'clamp(48px,6vw,72px) 0' }}>
          <motion.div initial={{ opacity:0,y:20 }} whileInView={{ opacity:1,y:0 }} viewport={{ once:true }} transition={{ duration:0.6,ease }}
            ...
            <p style={{ fontFamily:'Inter,sans-serif',fontSize:14,color:'#718096',fontWeight:300,lineHeight:1.7,marginBottom:24 }}>Start free — no credit card needed. Upgrade when you're ready.</p>
