            ...
            : cities.slice(0, 10).map((city, i) => (
                ...
                >
                  <Link href={`/schools?city=${city.value}`} className="card-hover p-5 flex flex-col items-center gap-2 text-center group block">
                    ...
                    <span className="font-display font-bold text-white text-sm group-hover:text-orange-400 transition-colors">{city.label}</span>
                    <span className="flex items-center gap-1 text-navy-400 text-xs"><MapPin className="w-3 h-3" /> View Schools</span>
                  </Link>
                </motion.div>
              ))
          }
    ...
    <section ref={ref} className="section">
      <div className="container-xl mx-auto">
        <motion.div
          ...
          style={{ background: 'linear-gradient(135deg, #151D52 0%, #0F1640 40%, #1a0d00 100%)', border: '1px solid rgba(255,92,0,0.2)' }}
        >
          ...
          <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-8">
            <div className="max-w-xl">
              ...
              <h2 className="font-display font-bold text-4xl md:text-5xl text-white mb-4">
                Talk to an Expert<br /><span className="text-gradient">Education Counsellor</span>
              </h2>
              <p className="text-navy-200 text-lg leading-relaxed mb-6">
                Confused about which board to choose? Our experts help 500+ families every month — completely free.
              </p>
              <div className="flex flex-wrap gap-4">
                {['CBSE vs ICSE vs IB guidance', 'School shortlisting help', 'Admission document checklist'].map(f => (
                  <div key={f} className="flex items-center gap-2 text-navy-300 text-sm">
            ...
            <div className="card p-8 w-full md:w-80 flex-shrink-0 text-center">
              ...
              <p className="text-navy-300 text-sm mb-6">Mon–Sat · 9 AM – 7 PM</p>
              <Link href="/counselling" className="btn-primary w-full justify-center block">Book Now — It&apos;s Free</Link>
    ...
    <section ref={ref} className="section bg-navy-950">
      <div className="container-xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={inView ? { opacity: 1, y: 0 } : {}} className="text-center mb-12">
          ...
          <h2 className="section-title mb-4">Trusted by <span className="text-gradient">1 Lakh+ Parents</span></h2>
        </motion.div>
          ...
          {TESTIMONIALS.map((t, i) => (
            ...
            >
              <div className="flex gap-1">
                {Array.from({ length: t.stars }).map((_, s) => <Star key={s} className="w-4 h-4 text-orange-400 fill-orange-400" />)}
              </div>
              <p className="text-navy-200 text-sm leading-relaxed italic">&ldquo;{t.text}&rdquo;</p>
      ...
      <div className="container-xl mx-auto">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          >
            ...
            <h3 className="font-display font-bold text-3xl text-white mb-3">Get Qualified<br />Admission Leads</h3>
            <p className="text-navy-300 mb-6 leading-relaxed">
              List your school free. Get matched with parents actively seeking admission. Buy only the leads you want.
            </p>
            ...
            <Link href="/register?role=school" className="btn-primary inline-flex">
          ...
          >
            {[
              { plan: 'Free',     price: '₹0',     leads: '5 leads/mo',   hot: false },
              { plan: 'Silver',   price: '₹2,999', leads: '25 leads/mo',  hot: false },
              { plan: 'Gold',     price: '₹5,999', leads: '75 leads/mo',  hot: true  },
              { plan: 'Platinum', price: '₹9,999', leads: 'Unlimited',    hot: false },
            ].map((p) => (
              <div key={p.plan} className={`card p-5 flex items-center justify-between gap-4 ${p.hot ? 'border-orange-500/40 shadow-orange-sm' : ''}`}>
                <div>
            ...
            <Link href="/pricing" className="btn-secondary text-center justify-center">View Full Pricing <ArrowRight className="w-4 h-4" /></Link>
          </motion.div>
        </div>
      </div>
    </section>
    ...
    <section ref={ref} className="section bg-navy-950">
      <div className="container-xl mx-auto">
        <div className="flex items-end justify-between mb-12">
          ...
          <Link href="/blog" className="btn-ghost text-orange-400 hidden sm:flex">All Articles <ArrowRight className="w-4 h-4" /></Link>
        </div>
