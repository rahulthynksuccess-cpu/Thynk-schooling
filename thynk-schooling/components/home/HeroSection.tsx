...
export function HeroSection() {
  ...
  return (
    <section style={{
      ...
      <div style={{ position:'absolute', inset:0, backgroundImage:'radial-gradient(rgba(184,134,11,0.05) 1px, transparent 1px)', backgroundSize:'28px 28px', pointerEvents:'none', zIndex:0 }} />
      <div style={{ position:'absolute', top:'-10%', right:'-5%', width:'800px', height:'800px', background:'radial-gradient(circle, rgba(184,134,11,0.09) 0%, transparent 65%)', pointerEvents:'none', zIndex:0 }} />
      <div style={{ position:'absolute', bottom:'10%', left:'-8%', width:'500px', height:'500px', background:'radial-gradient(circle, rgba(10,95,85,0.04) 0%, transparent 70%)', pointerEvents:'none', zIndex:0 }} />

      <div style={{ width:'100%', maxWidth:'var(--container-width,1600px)', margin:'0 auto', padding:'60px clamp(20px,5vw,80px)', position:'relative', zIndex:1 }}>
        <div style={{ display:'grid', gridTemplateColumns:'minmax(0,1.15fr) minmax(0,0.85fr)', gap:'clamp(40px,6vw,100px)', alignItems:'center' }}>
          ...
          <div>
            ...
            <motion.h1 {...fade(0.12)} style={{ fontFamily:'var(--font-serif,"Cormorant Garamond"),Georgia,serif', fontWeight:700, fontSize:'clamp(3.5rem, 7vw, var(--hero-h1-size,108px))', lineHeight:.90, letterSpacing:'-3px', color:'var(--hero-h1-color,#0D1117)', marginBottom:'28px' }}>
              {ct.h1Line1 || 'Find the'}
            ...
            <motion.p {...fade(0.22)} style={{ fontFamily:'var(--font-sans,Inter),sans-serif', fontSize:'clamp(16px,2.2vw,var(--hero-sub-size,19px))', fontWeight:'var(--hero-sub-weight,300)' as any, color:'var(--hero-sub-color,#4A5568)', lineHeight:1.75, maxWidth:'500px', marginBottom:'40px' }}>
            ...
            <motion.form {...fade(0.30)} onSubmit={handleSearch} style={{ marginBottom:'32px' }}>
              <div style={{ display:'flex', flexDirection:'column', background:'#fff', border:'1.5px solid rgba(13,17,23,0.13)', borderRadius:'14px', overflow:'hidden', maxWidth:'560px', boxShadow:'0 4px 24px rgba(13,17,23,0.07), 0 1px 3px rgba(13,17,23,0.06)' }}>
            ...
            <motion.div {...fade(0.38)} style={{ display:'flex', flexWrap:'wrap', gap:'20px', alignItems:'center', marginBottom:'32px' }}>
              {[
            ...
            {/* Social proof */}
            <motion.div {...fade(0.44)} style={{ display:'flex', alignItems:'center', gap:'16px', padding:'14px 18px', background:'rgba(255,255,255,0.75)', backdropFilter:'blur(12px)', borderRadius:'12px', border:'1px solid rgba(13,17,23,0.08)', maxWidth:'420px' }}>
              <div style={{ display:'flex' }}>
                {['P','R','A','S'].map((l,i) => (
                  <div key={i} style={{ width:'30px', height:'30px', borderRadius:'50%', background:`hsl(${i*45+30},35%,72%)`, border:'2px solid #fff', marginLeft:i>0?'-7px':'0', display:'flex', alignItems:'center', justifyContent:'center', fontFamily:'Cormorant Garamond,serif', fontWeight:700, fontSize:'12px', color:'#0D1117', position:'relative', zIndex:4-i }}>{l}</div>
                ))}
              </div>
              <div>
          ...
          {/* RIGHT */}
          <motion.div initial={{ opacity:0, scale:.95, x:30 }} animate={{ opacity:1, scale:1, x:0 }} transition={{ duration:.8, delay:.2, ease:[.22,1,.36,1] }}
            ...
            <div style={{ width:'100%', aspectRatio:'4/3', borderRadius:'24px', overflow:'hidden', boxShadow:'0 32px 80px rgba(13,17,23,0.14), 0 8px 24px rgba(184,134,11,0.08)', border:'1px solid rgba(13,17,23,0.08)', position:'relative' }}>
              <img src={heroImage} alt="Happy students in school" style={{ width:'100%', height:'100%', objectFit:'cover', display:'block' }} />
              <div style={{ position:'absolute', inset:0, background:'linear-gradient(to top, rgba(13,17,23,0.3) 0%, transparent 55%)', pointerEvents:'none' }} />
              <div style={{ position:'absolute', bottom:'20px', left:'20px', background:'rgba(255,255,255,0.96)', backdropFilter:'blur(12px)', borderRadius:'14px', padding:'14px 18px', boxShadow:'0 8px 32px rgba(13,17,23,0.14)', display:'flex', alignItems:'center', gap:'14px', border:'1px solid rgba(13,17,23,0.06)' }}>
              ...
              <div style={{ position:'absolute', top:'20px', right:'20px', background:'rgba(184,134,11,0.92)', backdropFilter:'blur(8px)', borderRadius:'100px', padding:'7px 15px', fontFamily:'Inter,sans-serif', fontSize:'11px', fontWeight:600, color:'#fff', display:'flex', alignItems:'center', gap:'6px', boxShadow:'0 4px 16px rgba(184,134,11,0.3)' }}>
                <Sparkles style={{ width:12, height:12 }} /> AI Powered
              </div>
              <div style={{ position:'absolute', bottom:'20px', right:'20px', background:'rgba(13,17,23,0.88)', backdropFilter:'blur(12px)', borderRadius:'12px', padding:'10px 14px', border:'1px solid rgba(255,255,255,0.1)', display:'flex', alignItems:'center', gap:'8px' }}>
                <div style={{ display:'flex', gap:'2px' }}>
                  {[1,2,3,4,5].map(s => <Star key={s} style={{ width:'10px', height:'10px', fill:'#E8C547', color:'#E8C547' }} />)}
                </div>
                <span style={{ fontFamily:'Inter,sans-serif', fontSize:'11px', color:'rgba(250,247,242,0.7)', fontWeight:300 }}>4.8 avg rating</span>
              </div>
            </div>
          </motion.div>
        </div>
