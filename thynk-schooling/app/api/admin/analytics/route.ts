...
// Cover all successful payment statuses across gateways
  ...
  try {
    const [
      ...
      db.query(`
        ...
        GROUP BY s.school_type ORDER BY leads DESC LIMIT 6
      `).catch(() => ({ rows: [] })),

      db.query(`
        SELECT s.city AS type,
               COUNT(DISTINCT s.id)::int                                                             AS school_count,
               COUNT(l.id)::int                                                                      AS leads,
               COUNT(a.id)::int                                                                      AS applications,
               COALESCE(ROUND(COUNT(a.id)::numeric / NULLIF(COUNT(l.id),0)*100),0)::int             AS conversion_pct
      ...
      db.query(`
        ...
        GROUP BY dow, week_ago ORDER BY week_ago DESC, dow
      `).catch(() => ({ rows: [] })),

      db.query(`
        SELECT
          CASE
            WHEN class_applying_for ILIKE ANY(ARRAY['Nursery','LKG','UKG','Pre-K','KG','Pre Primary']) THEN 'Nursery–KG'
            WHEN class_applying_for ILIKE ANY(ARRAY['1','2','3','4','5','Grade 1','Grade 2','Grade 3','Grade 4','Grade 5','Class 1','Class 2','Class 3','Class 4','Class 5']) THEN 'Grade 1–5'
            WHEN class_applying_for ILIKE ANY(ARRAY['6','7','8','9','10','Grade 6','Grade 7','Grade 8','Grade 9','Grade 10','Class 6','Class 7','Class 8','Class 9','Class 10']) THEN 'Grade 6–10'
            WHEN class_applying_for ILIKE ANY(ARRAY['11','12','Grade 11','Grade 12','Class 11','Class 12']) THEN 'Grade 11–12'
            ELSE 'Other'
          END AS class_group,
          COUNT(*)::int AS count
        FROM leads
        WHERE class_applying_for IS NOT NULL AND class_applying_for <> ''
        GROUP BY class_group ORDER BY count DESC
      `).catch(() => ({ rows: [] })),
    ...
    weeklyActivity.rows.forEach((r: any) => {
      const idx = 3 - Number(r.week_ago)
      const dow = Number(r.dow)
      if (idx >= 0 && idx < 4 && dow >= 0 && dow < 7) heatMatrix[idx][dow] = Number(r.count)
    })
    ...
    const classPcts  = classBreakdown.rows.map((r: any) => ({
      name: r.class_group, value: Number(r.count),
    ...
    const cityPcts  = parentCityBreakdown.rows.map((r: any) => ({
      name: r.city, value: Number(r.count),
    ...
    return NextResponse.json({
      ...
      parentStats: {
        ...
        classPcts, cityPcts, totalParents,
