...
export async function GET(req: NextRequest) {
  try {
    ...
    const users = rows.rows.map((r: any) => ({
      ...
      email: r.email || null,
      role: r.role,
      // Users page reads u.school — map schoolName to school
    ...
    return Response.json({ users: [], data: [], total: 0, totalPages: 1, stats: { total: 0, parents: 0, schools: 0, suspended: 0 } })
  }
}

export async function DELETE(req: NextRequest) {
  try {
    const id = new URL(req.url).searchParams.get('id') || req.url.split('/').pop()
