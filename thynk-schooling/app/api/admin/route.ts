...
async function saveSubPlan(req: NextRequest) {
  await ensureSubPlansTable()
  const body = await req.json()
  const { planKey, name, description, price, leadsPerMonth, features, isHot, cta, sortOrder, isActive, isFeaturedListing, featuredListingDays } = body
  ...
  const res = await db.query(
    `INSERT INTO subscription_plans (plan_key,name,description,price_paise,leads_per_month,features,is_hot,cta,sort_order,is_active,is_featured_listing,featured_listing_days)
     ...
     RETURNING *`,
    [planKey, name, description||'', price??0, leadsPerMonth??0, JSON.stringify(features??[]), isHot??false, cta||'Get Started', sortOrder??0, isActive??true, isFeaturedListing??false, featuredListingDays??0]
  )
...
async function updateSubPlan(req: NextRequest) {
  ...
  const body = await req.json()
  const sets: string[] = []; const params: any[] = []
  const map: Record<string,string> = { name:'name', description:'description', price:'price_paise', leadsPerMonth:'leads_per_month', isHot:'is_hot', cta:'cta', sortOrder:'sort_order', isActive:'is_active', isFeaturedListing:'is_featured_listing', featuredListingDays:'featured_listing_days' }
