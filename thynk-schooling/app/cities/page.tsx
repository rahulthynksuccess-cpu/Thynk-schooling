...
const DEFAULT_CITIES = [
  ...
  ['Ghaziabad','ghaziabad','Uttar Pradesh'],['Howrah','howrah','West Bengal'],
  ...
  ['Bikaner','bikaner','Rajasthan'],['Nellore','nellore','Andhra Pradesh'],
...
export default function AdminCitiesPage() {
  ...
  const [filterState, setFilterState] = useState('__all__')
  ...
  // Sorted unique state list derived from cities
  const stateOptions = useMemo(() => {
    const unique = Array.from(new Set(cities.map(c => c.state).filter(Boolean))).sort()
    return unique
  }, [cities])

  // Cities filtered by selected state
  const visibleCities = useMemo(() => {
    if (filterState === '__all__') return cities
    return cities.filter(c => c.state === filterState)
  }, [cities, filterState])
  ...
  return (
    <AdminLayout title="SEO Cities Manager" subtitle="Manage all city pages and footer city links — used for SEO">
      ...
      <div style={{ display:'flex', flexWrap:'wrap', gap:8, marginBottom:16, alignItems:'center' }}>
        <span style={{ color:'var(--admin-text-muted,rgba(255,255,255,0.4))', fontSize:13, fontFamily:'DM Sans,sans-serif' }}>
          {visibleCities.length} / {cities.length} cities
        </span>
        ...
        >
          <option value="__all__">All States ({cities.length})</option>
          {stateOptions.map(s => (
            <option key={s} value={s}>
              {s} ({cities.filter(c => c.state === s).length})
            </option>
          ))}
        </select>
      ...
      <div style={{ display:'flex', gap:8, marginBottom:16, alignItems:'center', background:'var(--admin-card-bg,rgba(255,255,255,0.04))', border:'1px solid rgba(184,134,11,0.2)', borderRadius:10, padding:'10px 14px' }}>
        ...
        <select value={newState} onChange={e => setNewState(e.target.value)} style={{ ...sel, width:180 }}>
      ...
      <div style={{ display:'flex', flexDirection:'column', gap:3 }}>
        {loading ? (
          <div style={{ textAlign:'center', padding:40, color:'var(--admin-text-faint,rgba(255,255,255,0.3))', fontFamily:'DM Sans,sans-serif' }}>Loading…</div>
        ) : visibleCities.length === 0 ? (
          <div style={{ textAlign:'center', padding:40, color:'var(--admin-text-faint,rgba(255,255,255,0.3))', fontFamily:'DM Sans,sans-serif' }}>No cities found for this state.</div>
        ) : (
          visibleCities.map(city => (
            <div key={city.slug} style={{ display:'grid', gridTemplateColumns:'1fr 1fr 160px 36px', gap:8, alignItems:'center', background:'var(--admin-card-bg,rgba(255,255,255,0.03))', border:'1px solid var(--admin-border,rgba(255,255,255,0.07))', borderRadius:8, padding:'7px 12px' }}>
              <input value={city.name} onChange={e => update(city.slug, 'name', e.target.value)} style={{ ...inp, padding:'5px 9px', fontSize:12 }} />
              <select value={city.state} onChange={e => update(city.slug, 'state', e.target.value)} style={{ ...sel, padding:'5px 9px', fontSize:12 }}>
                {stateOptions.map(s => <option key={s} value={s}>{s}</option>)}
                {!stateOptions.includes(city.state) && city.state && (
