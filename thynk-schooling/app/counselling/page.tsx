...
export default function CounsellingPage() {
  const ct = useContent('counselling') ?? {}
  ...
  const benefits = [
    { icon:'🎓', title: g('benefit1Title','Board Selection'),     desc: g('benefit1Desc','CBSE vs ICSE vs IB — our experts help you pick what suits your child') },
    { icon:'🏫', title: g('benefit2Title','School Shortlisting'), desc: g('benefit2Desc','Personalised shortlist of 5–10 schools based on budget, location and values') },
    { icon:'📋', title: g('benefit3Title','Admission Roadmap'),   desc: g('benefit3Desc','Step-by-step checklist with deadlines for every school you apply to') },
    { icon:'💰', title: g('benefit4Title','Fee & Scholarship'),   desc: g('benefit4Desc','Navigate fee structures, hidden costs and scholarship opportunities') },
  ]

  const testimonials = [
    { name: g('test1Name','Priya Sharma'), city: g('test1City','Delhi'),     text: g('test1Text','The counsellor understood our exact needs and shortlisted 6 perfect schools. My daughter got admission in her first choice!'), stars:5 },
    { name: g('test2Name','Rahul Mehta'),  city: g('test2City','Mumbai'),    text: g('test2Text','Amazing service. Cleared all my doubts about IB vs CBSE in one 45-minute call. Totally free and no pressure at all.'),       stars:5 },
    { name: g('test3Name','Anita Singh'), city: g('test3City','Bangalore'), text: g('test3Text','Helped us understand the admission timeline perfectly. Got our son into a top ICSE school with their guidance.'),               stars:5 },
  ]
        ...
        <section style={{background:'linear-gradient(160deg,#0D1117 0%,#1C2333 60%,#0D1117 100%)',padding:'clamp(80px,12vw,140px) 0',position:'relative',overflow:'hidden',minHeight:'80vh',display:'flex',alignItems:'center'}}>
          ...
          <div style={{maxWidth:'1600px',margin:'0 auto',padding:'0 clamp(24px,5vw,80px)',position:'relative',zIndex:1,width:'100%'}}>
            <div style={{display:'grid',gridTemplateColumns:'1fr 420px',gap:'clamp(48px,6vw,80px)',alignItems:'center'}}>
              <div>
                <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{duration:.6}}>
                  <div style={{display:'inline-flex',alignItems:'center',gap:8,fontFamily:'Inter,sans-serif',fontSize:11,fontWeight:600,letterSpacing:'.18em',textTransform:'uppercase',color:'#E8C547',marginBottom:24}}>
                    <span style={{width:22,height:1,background:'#B8860B',display:'block'}}/>{g('eyebrow','100% Free Service')}
                  </div>
                </motion.div>
                <motion.h1 initial={{opacity:0,y:32}} animate={{opacity:1,y:0}} transition={{duration:.75,delay:.08,ease:[.22,1,.36,1]}}
                  ...
                  {g('h1Line1','Talk to an Expert')}
                  <em className="shimmer-text" style={{display:'block',fontStyle:'italic'}}>{g('h1Italic','Education Counsellor')}</em>
                </motion.h1>
                <motion.p initial={{opacity:0,y:16}} animate={{opacity:1,y:0}} transition={{duration:.65,delay:.2}}
                  ...
                  {g('subtext','Our experts have helped 500+ families every month find the right school. Free, with zero pressure.')}
                </motion.p>
                <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:14}}>
                  {benefits.map((b,i)=>(
                    <motion.div key={i} initial={{opacity:0,x:-20}} animate={{opacity:1,x:0}} transition={{delay:.3+i*.08,duration:.55}}
                      style={{background:'rgba(255,255,255,0.05)',border:'1px solid rgba(255,255,255,0.08)',borderRadius:14,padding:'clamp(16px,2.5vw,24px)',backdropFilter:'blur(8px)',transition:'all .22s'}}
              ...
              {/* Booking form */}
              <motion.div initial={{opacity:0,x:32,scale:.96}} animate={{opacity:1,x:0,scale:1}} transition={{duration:.75,delay:.15}}>
                <div style={{background:'rgba(255,255,255,0.96)',backdropFilter:'blur(20px)',borderRadius:22,padding:'clamp(28px,4vw,40px)',boxShadow:'0 40px 100px rgba(13,17,23,0.4)',border:'1px solid rgba(255,255,255,0.8)'}}>
                  <div style={{fontFamily:'"Cormorant Garamond",serif',fontWeight:700,fontSize:26,color:'#0D1117',marginBottom:6}}>{g('formTitle','Book a Free Session')}</div>
                  <div style={{fontFamily:'Inter,sans-serif',fontSize:12,color:'#718096',marginBottom:24,fontWeight:300}}>{g('formSubtitle','Mon–Sat · 9 AM – 7 PM · Hindi & English')}</div>
                  <form onSubmit={handleSubmit} style={{display:'flex',flexDirection:'column',gap:14}}>
                    ...
                    <button type="submit" disabled={mutation.isPending}
                      ...
                      {mutation.isPending?<><Loader2 style={{width:16,height:16}} className="animate-spin"/>Booking…</>:<>{g('ctaBtn','Book My Free Session')} <ArrowRight style={{width:15,height:15}}/></>}
                    </button>
                  </form>
                  <div style={{fontFamily:'Inter,sans-serif',fontSize:11,color:'#A0ADB8',textAlign:'center',marginTop:12}}>{g('formDisclaimer','No sales calls · No obligation · 100% free')}</div>
                </div>
              </motion.div>
            </div>
          </div>
          ...
          <div style={{maxWidth:'1600px',margin:'0 auto',padding:'0 clamp(24px,5vw,80px)'}}>
            ...
            <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:'clamp(12px,2vw,20px)'}} className="grid-3">
              {testimonials.map((t,i)=>(
                <motion.div key={i} initial={{opacity:0,y:32,scale:.96}} whileInView={{opacity:1,y:0,scale:1}} viewport={{once:true}} transition={{delay:i*.12,duration:.65}}
