...
// Axis tick using theme text color — prevents white-on-white issue
const axisTick = { fill: T.t3, fontSize: 11, fontFamily: 'Plus Jakarta Sans,sans-serif' }
    ...
    <div style={{ display: 'flex', gap: 3, background: 'rgba(255,255,255,0.04)', borderRadius: 10, padding: 3, border: `1px solid ${T.border}` }}>
      {tabs.map(t => (
        <button key={t} onClick={() => onChange(t)}
          style={{ padding: '6px 14px', borderRadius: 8, border: 'none', cursor: 'pointer', fontFamily: 'Plus Jakarta Sans,sans-serif', fontSize: 12, fontWeight: active === t ? 700 : 500, background: active === t ? T.gold : 'transparent', color: active === t ? '#000' : T.t2, transition: 'all 0.15s' }}>
          {t}
        </button>
      ))}
    </div>
                  ...
                  : (data?.recentLeads || []).length === 0
                    ...
                    : (data?.recentLeads || []).map((l: any) => (
                      <tr key={l.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.03)', transition: 'background .15s' }}
                        ...
                        <td style={{ padding: '11px 16px', fontFamily: 'Plus Jakarta Sans,sans-serif', fontSize: 13, fontWeight: 700, color: T.green }}>
