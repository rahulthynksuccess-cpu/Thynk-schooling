...
const DEFAULT: Record<string, any> = {
  ...
  /* Global typography */
  sizeBase: 15, sizeH1: 80, sizeH2: 56, sizeH3: 36, sizeH4: 22, sizeSmall: 13,
  ...
  heroH1Size: 80, heroH1Color: '#0D1117', heroItalicColor: '#B8860B',
  heroSubSize: 15, heroSubColor: '#4A5568', heroSubWeight: 300,
...
const PAGES = [
  ...
  { label:'🏫 Schools Page',    sections:[
    { key:'schools-list',label:'School Listing'   },
    { key:'school-profile',label:'School Profile' },
  ]},
  { label:'🔍 Counselling',     sections:[{ key:'counselling', label:'Counselling Page' }]},
  { label:'⇌ Compare',         sections:[{ key:'compare',     label:'Compare Page'     }]},
  { label:'💰 Pricing',         sections:[{ key:'pricing',     label:'Pricing Page'     }]},
  { label:'📝 Blog',            sections:[{ key:'blog',        label:'Blog Page'        }]},
  { label:'🔐 Login / Register',sections:[{ key:'login',       label:'Login & Register' }]},
  { label:'📊 Dashboard',       sections:[{ key:'dashboard',   label:'Dashboard'        }]},
]
      ...
      <div style={{ display:'grid', gridTemplateColumns:'220px 1fr', gap:'16px' }}>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:'14px' }}>
          ...
          <div style={{ background:'#fff', border:'1px solid rgba(13,17,23,0.08)', borderRadius:'12px', overflow:'hidden' }}>
            ...
            <div style={{ padding:'24px', background:String(theme.ivory||'#FAF7F2') }}>

              {/* Navbar preview */}
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'12px 20px', background:String(theme.navBg||'rgba(250,247,242,0.95)'), borderRadius:'8px', marginBottom:'16px', border:'1px solid rgba(13,17,23,0.07)' }}>
                <div style={{ display:'flex', alignItems:'center', gap:'8px' }}>
                  <div style={{ width:28, height:28, borderRadius:'7px', background:String(theme.ink||'#0D1117'), display:'flex', alignItems:'center', justifyContent:'center', fontSize:'12px' }}>🎓</div>
                  <span style={{ fontFamily:`'${theme.fontSans||'Inter'}',sans-serif`, fontSize:'14px', fontWeight:600, color:String(theme.navLogoColor||'#0D1117') }}>Thynk Schooling</span>
                </div>
                <div style={{ display:'flex', gap:'20px' }}>
                  {['Find Schools','Compare','Counselling'].map(l=>(
                    <span key={l} style={{ fontFamily:`'${theme.fontSans||'Inter'}',sans-serif`, fontSize:`${theme.navSize||13}px`, color:String(theme.navColor||'#4A5568'), fontWeight:Number(theme.navWeight||400) }}>{l}</span>
                  ))}
                </div>
                <div style={{ padding:'8px 18px', borderRadius:`${theme.btnRadius||6}px`, background:String(theme.btnPrimaryBg||'#0D1117'), color:String(theme.btnPrimaryColor||'#FAF7F2'), fontFamily:`'${theme.fontSans||'Inter'}',sans-serif`, fontSize:`${theme.btnSize||13}px`, fontWeight:500 }}>List Your School</div>
              </div>

              {/* Hero preview */}
              <div style={{ padding:'32px 24px', background:`linear-gradient(160deg, ${theme.heroBg||'#FAF7F2'} 55%, ${theme.heroBgGrad||'#F0EAD6'} 100%)`, borderRadius:'10px', marginBottom:'14px' }}>
                <div style={{ fontFamily:`'${theme.fontSans||'Inter'}',sans-serif`, fontSize:`${theme.heroEyebrowSize||10}px`, fontWeight:600, letterSpacing:'.18em', textTransform:'uppercase', color:String(theme.heroEyebrowColor||'#B8860B'), marginBottom:'14px' }}>
                  — AI-Powered School Matching — Free for Parents
                </div>
                <div style={{ fontFamily:`'${theme.fontSerif||'Cormorant Garamond'}',Georgia,serif`, fontWeight:Number(theme.weightHeading||700), fontSize:`${Math.min(theme.heroH1Size||80, 56)}px`, lineHeight:.96, letterSpacing:'-2px', color:String(theme.heroH1Color||'#0D1117'), marginBottom:'12px' }}>
                  Find the <em style={{ display:'block', fontStyle:'italic', color:String(theme.heroItalicColor||'#B8860B') }}>Perfect School</em>
                </div>
                <p style={{ fontFamily:`'${theme.fontSans||'Inter'}',sans-serif`, fontSize:`${theme.heroSubSize||15}px`, color:String(theme.heroSubColor||'#4A5568'), fontWeight:Number(theme.heroSubWeight||300), lineHeight:1.75, maxWidth:'380px', marginBottom:'16px' }}>
                  Search, compare & apply to <strong>12,000+ verified schools</strong> across 35+ Indian cities.
                </p>
                <div style={{ display:'flex', gap:'10px' }}>
                  <span style={{ padding:'10px 22px', borderRadius:`${theme.btnRadius||6}px`, background:String(theme.btnGoldBg||'#B8860B'), color:String(theme.btnGoldColor||'#fff'), fontFamily:`'${theme.fontSans||'Inter'}',sans-serif`, fontSize:`${theme.btnSize||13}px`, fontWeight:500 }}>Search Schools</span>
                  <span style={{ padding:'9px 20px', borderRadius:`${theme.btnRadius||6}px`, border:'1.5px solid rgba(13,17,23,0.15)', color:String(theme.ink||'#0D1117'), fontFamily:`'${theme.fontSans||'Inter'}',sans-serif`, fontSize:`${theme.btnSize||13}px` }}>Free Counselling →</span>
                </div>
              </div>

              {/* Stats preview */}
              <div style={{ display:'grid', gridTemplateColumns:'repeat(5,1fr)', background:String(theme.statsBg||'#F5F0E8'), borderRadius:`${theme.radius||12}px`, marginBottom:'14px', overflow:'hidden', border:'1px solid rgba(13,17,23,0.06)' }}>
                {[['12,000+','Verified Schools'],['1 Lakh+','Happy Parents'],['35+','Cities'],['98%','Satisfaction'],['4.8★','Rating']].map(([n,l],i)=>(
                  <div key={l} style={{ padding:'16px 0', textAlign:'center', borderRight:i<4?'1px solid rgba(13,17,23,0.06)':'none' }}>
                    <div style={{ fontFamily:`'${theme.fontSerif||'Cormorant Garamond'}',Georgia,serif`, fontWeight:700, fontSize:`${theme.statNumSize||38}px`, color:String(theme.statNumColor||'#0D1117'), lineHeight:1 }}>{n}</div>
                    <div style={{ fontFamily:`'${theme.fontSans||'Inter'}',sans-serif`, fontSize:`${theme.statLabelSize||12}px`, color:String(theme.statLabelColor||'#718096'), marginTop:'4px' }}>{l}</div>
                  </div>
                ))}
              </div>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:'12px', marginBottom:'14px' }}>
                {[{name:'Delhi Public School',city:'New Delhi',board:'CBSE',fees:'₹1.2L/yr'},
                  {name:'Ryan International',city:'Mumbai',board:'ICSE',fees:'₹95K/yr'},
                  {name:'Heritage School',city:'Kolkata',board:'IB',fees:'₹2.8L/yr'},
                ].map(s=>(
                  <div key={s.name} style={{ background:String(theme.schoolCardBg||'#fff'), border:`1px solid ${theme.schoolCardBorder||'rgba(13,17,23,0.08)'}`, borderRadius:`${theme.radius||12}px`, overflow:'hidden' }}>
                    <div style={{ height:60, background:'linear-gradient(135deg,rgba(184,134,11,0.08),rgba(184,134,11,0.03))' }} />
                    <div style={{ padding:'12px' }}>
                      <div style={{ fontFamily:`'${theme.fontSerif||'Cormorant Garamond'}',Georgia,serif`, fontSize:`${theme.schoolNameSize||17}px`, fontWeight:700, color:String(theme.schoolNameColor||'#0D1117'), marginBottom:'4px' }}>{s.name}</div>
                      <div style={{ fontFamily:`'${theme.fontSans||'Inter'}',sans-serif`, fontSize:`${theme.schoolMetaSize||12}px`, color:String(theme.schoolMetaColor||'#718096') }}>{s.city} · {s.board} · {s.fees}</div>
                    </div>
                  </div>
                ))}
              </div>
              ...
              {/* Footer preview */}
              <div style={{ background:String(theme.footerBg||'#0D1117'), borderRadius:'8px', padding:'16px 20px' }}>
                <div style={{ fontFamily:`'${theme.fontSans||'Inter'}',sans-serif`, fontSize:`${theme.footerHeadingSize||11}px`, fontWeight:600, letterSpacing:'1.5px', textTransform:'uppercase', color:String(theme.footerHeadingColor||'rgba(250,247,242,0.55)'), marginBottom:'6px' }}>THYNK SCHOOLING</div>
                <div style={{ fontFamily:`'${theme.fontSans||'Inter'}',sans-serif`, fontSize:`${theme.footerTextSize||13}px`, color:String(theme.footerTextColor||'rgba(250,247,242,0.4)'), fontWeight:300 }}>
                  Find the perfect school for your child — 12,000+ verified schools across India.
                </div>
              </div>
            </div>
          </div>
