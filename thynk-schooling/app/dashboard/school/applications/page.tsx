...
function ApplicationsList() {
  ...
  useEffect(() => {
    fetch('/api/applications',{cache:'no-store'}).then(r=>r.json()).then(d=>setApps(d.applications||d.data||[])).catch(()=>{}).finally(()=>setLoading(false))
  },[])
  const statusColor: Record<string,string> = {submitted:'#B8860B',under_review:'#3B82F6',accepted:'#10B981',rejected:'#EF4444',waitlisted:'#8B5CF6'}
  return (
    <div style={{background:'#fff',borderRadius:14,border:'1px solid rgba(13,17,23,0.08)',overflow:'hidden'}}>
      <div style={{padding:'16px 20px',borderBottom:'1px solid rgba(13,17,23,0.07)',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
        <span style={{fontFamily:'DM Sans,sans-serif',fontSize:14,fontWeight:600,color:'#0D1117'}}>Admission Applications</span>
        <span style={{fontSize:12,color:'#718096',fontFamily:'DM Sans,sans-serif'}}>{apps.length} total</span>
      </div>
      {loading ? <div style={{padding:40,textAlign:'center',color:'#718096',fontFamily:'DM Sans,sans-serif'}}>Loading…</div> :
      apps.length === 0 ? <div style={{padding:40,textAlign:'center'}}><div style={{fontSize:40,marginBottom:12}}>📝</div><p style={{fontFamily:'DM Sans,sans-serif',color:'#718096'}}>No applications yet.</p></div> :
      <table style={{width:'100%',borderCollapse:'collapse'}}>
        <thead><tr style={{background:'#F9F8F6'}}>{['Applicant','Class','Applied On','Status'].map(h=><th key={h} style={{padding:'10px 16px',textAlign:'left',fontSize:11,fontWeight:600,color:'#718096',fontFamily:'DM Sans,sans-serif',letterSpacing:'.08em',textTransform:'uppercase'}}>{h}</th>)}</tr></thead>
        <tbody>{apps.map((a:any,i:number)=><tr key={a.id||i} style={{borderTop:'1px solid rgba(13,17,23,0.05)'}}>
          <td style={{padding:'12px 16px',fontFamily:'DM Sans,sans-serif',fontSize:13,color:'#0D1117',fontWeight:500}}>{a.studentName||a.student_name||'—'}</td>
          <td style={{padding:'12px 16px',fontSize:12,color:'#4A5568',fontFamily:'DM Sans,sans-serif'}}>{a.classApplied||a.class_applied||'—'}</td>
          <td style={{padding:'12px 16px',fontSize:12,color:'#718096',fontFamily:'DM Sans,sans-serif'}}>{a.createdAt?new Date(a.createdAt).toLocaleDateString('en-IN'):'—'}</td>
          <td style={{padding:'12px 16px'}}><span style={{padding:'3px 10px',borderRadius:100,fontSize:11,fontWeight:600,background:`${statusColor[a.status]||'#B8860B'}18`,color:statusColor[a.status]||'#B8860B',fontFamily:'DM Sans,sans-serif'}}>{a.status||'Submitted'}</span></td>
        </tr>)}</tbody>
      </table>}
    </div>
  )
}
export default function ApplicationsPage() { return <SchoolLayout title="Applications"><ApplicationsList /></SchoolLayout> }