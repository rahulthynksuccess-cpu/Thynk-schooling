...
const SOURCE_BADGE: Record<string, { label: string; bg: string; color: string; icon: string }> = {
  ...
  geo:     { label: 'Nearby',   bg: '#DBEAFE', color: '#2563EB', icon: '📡' },
  search:  { label: 'Searched', bg: '#FEF3C7', color: '#D97706', icon: '🔍' },
}
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', zIndex: 999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      <div style={{ background: '#fff', borderRadius: 18, padding: '32px 28px', maxWidth: 420, width: '100%', boxShadow: '0 20px 60px rgba(0,0,0,0.25)' }}>
        ...
        <p style={{ fontSize: 14, color: '#6B7280', lineHeight: 1.65, marginBottom: 20 }}>
          ...
          The parent's name and phone will be revealed instantly.
        </p>
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          <button onClick={onCancel} disabled={loading} style={{ flex: 1, padding: '11px', borderRadius: 10, background: '#F3F4F6', border: 'none', fontSize: 14, fontWeight: 600, color: '#374151', cursor: 'pointer' }}>
            Cancel
          </button>
          <button onClick={onConfirm} disabled={loading} style={{ flex: 2, padding: '11px', borderRadius: 10, background: '#F59E0B', border: 'none', fontSize: 14, fontWeight: 700, color: '#fff', cursor: loading ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
...
function LeadsContent() {
  ...
  if (isLoading) return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
      {Array.from({ length: 5 }).map((_, i) => <div key={i} style={{ height: 60, borderRadius: 10, background: '#F3F4F6' }} />)}
    </div>
  )
                      ...
                      <td style={{ padding: '13px 16px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                          ...
                          <div>
                            ...
                            {lead.childName && <div style={{ fontSize: 11, color: '#6B7280', marginTop: 1 }}>{lead.childName}</div>}
                          </div>
                        </div>
                      </td>
