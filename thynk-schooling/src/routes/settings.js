// src/routes/settings.js — Thynk Schooling
// Handles all dropdown_options CRUD with:
//   - ?includeInactive=true  → used by admin settings page
//   - ?parentValue=xxx       → used for cascading city dropdown
//   - POST / PUT / DELETE    → admin manages options
...
// Query params:
//   category        (required) — e.g. 'board', 'city'
//   parentValue     (optional) — filter cities by state value
//   includeInactive (optional) — 'true' shows inactive rows too (admin only)
...
// ── POST /api/settings/dropdown — Create new option (admin) ───
router.post('/dropdown', async (req, res) => {
  try {
    ...
    if (err.code === '23505') {
      return res.status(409).json({ error: 'A option with this value already exists in this category' });
    }
...
// ── PUT /api/settings/dropdown/:id — Update option (admin) ────
...
// ── DELETE /api/settings/dropdown/:id — Delete option (admin) ─
